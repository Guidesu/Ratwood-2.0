...
/datum/storyteller
	...
	/// Ratwood's direct preset cap; null falls back to preferred_gnoll_mode.
	var/gnoll_slot_cap
	...
	/// Roundstart event-control types spawned in addition to the normal hard-antag roll.
	var/list/forced_roundstart_antags = list()
	/// If populated, a guaranteed hard-antag roll is restricted to these event controls.
	var/list/guaranteed_roundstart_antags = list()
	/// Allows Bandits while other hard antagonists remain blocked.
	var/allow_bandits = FALSE
