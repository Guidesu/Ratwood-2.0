...
/datum/outfit/job/roguetown/adventurer/fisher/pre_equip(mob/living/carbon/human/H)
	..()
	// Fisher cosmetic title selection
	if(H.mind)
		...
		var/cosmetic_titles = list(
			...
			"Fisherman",
			"Angler")
		var/cosmetic_choice = input(H, "Select your fishing profession.", "Fishing Professions") as anything in cosmetic_titles
		
		switch(cosmetic_choice)
			if("Fisher")
				to_chat(H, span_notice("You are a Fisher, catching fish from the waters."))
				H.mind.cosmetic_class_title = "Fisher"
				H.adjust_skillrank(/datum/skill/labor/fishing, 1, TRUE)
			if("Fisherman")
				to_chat(H, span_notice("You are a Fisherman, making your living from the sea."))
				H.mind.cosmetic_class_title = "Fisherman"
				H.adjust_skillrank(/datum/skill/labor/fishing, 1, TRUE)
			if("Angler")
				to_chat(H, span_notice("You are an Angler, skilled with rod and line."))
