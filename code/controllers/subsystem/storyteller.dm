...
SUBSYSTEM_DEF(gamemode)
	...
	var/ruling_god
	/// Legacy display name read by Ratwood's stat panel.
	var/storyteller_name = "Gamemode"
...
/datum/controller/subsystem/gamemode/proc/spawn_extra_antags()
	...
	// Named preset threats are additional to the normal hard-antag roll.
	// Forced triggering bypasses weights/pop gates but still fails safely when
	// the event cannot assign a valid player.
	for(var/forced_event_type in preset.forced_roundstart_antags)
		var/datum/round_event_control/antagonist/solo/forced_event
		for(var/datum/round_event_control/antagonist/solo/candidate in event_pools[EVENT_TRACK_CHARACTER_INJECTION])
			if(istype(candidate, forced_event_type))
				forced_event = candidate
				break
		if(!forced_event || forced_event.occurrences)
			continue
		log_storyteller("Forcing preset roundstart antagonist [forced_event.name].")
		TriggerEvent(forced_event, TRUE)
	for(var/datum/round_event_control/antagonist/solo/ec in event_pools[EVENT_TRACK_CHARACTER_INJECTION])
		...
		if(ec.storyteller_antag_flags & STORYTELLER_ANTAG_VILLAIN)
			// High Intensity's apex roll is restricted to Lich/Vampire Lord;
			// Bandits remain a separate, optional supporting threat.
			if(preset.guaranteed_hard && preset.allow_bandits && istype(ec, /datum/round_event_control/antagonist/solo/bandits) && prob(50) && ec.canSpawnEvent(pop))
				log_storyteller("Spawning supporting roundstart Bandits alongside the apex antagonist.")
...
/datum/controller/subsystem/gamemode/proc/preset_gnoll_count(datum/storyteller/preset)
	if(!isnull(preset?.gnoll_slot_cap))
		return max(0, preset.gnoll_slot_cap)
...
/// gnoll) from the preset's own vars, plus any roundstart antag event whose datum the preset permits. Hard antags
...
/datum/controller/subsystem/gamemode/proc/storyteller_event_weight(datum/round_event_control/event, base_weight, storyteller_type = null)
	...
	if(!storyteller_type)
		return base_weight
	var/datum/storyteller/preset = storytellers?[storyteller_type]
	if(istype(event, /datum/round_event_control/antagonist/solo/bandits) && preset?.allow_bandits)
		return base_weight
...
/datum/controller/subsystem/gamemode/proc/story_antag_slot_cap(antag_datum, roundstart = FALSE, storyteller_type = null)
	...
	var/datum/storyteller/preset = storytellers?[storyteller_type]
	var/bandit_exception = ispath(antag_datum, /datum/antagonist/bandit) && preset?.allow_bandits
	if(!bandit_exception && storyteller_blocks_antag(storyteller_antag_flags, roundstart, storyteller_type))
		return 0
...
/datum/controller/subsystem/gamemode/proc/storyteller_guaranteed_events(list/valid_events)
	...
	for(var/datum/round_event_control/event as anything in valid_events)
		...
		if(event.storyteller_antag_flags & STORYTELLER_ANTAG_VILLAIN)
			if(length(preset.guaranteed_roundstart_antags) && !(event.type in preset.guaranteed_roundstart_antags))
				continue
			guaranteed_events[event] = valid_events[event]
	return guaranteed_events
...
/datum/controller/subsystem/gamemode/proc/refresh_alive_stats(roundstart = FALSE)
	...
	for(var/client/client in GLOB.clients)
		...
		if(ishuman(living))
			...
			if(human_mob.mind.assigned_role in GLOB.peasant_positions)
