...
/datum/controller/subsystem/vote/proc/initiate_vote(vote_type, initiator_key, vote_period)
	...
	if(!mode)
		...
		// The lobby vote is important enough to open for every connected client.
		// Do not rely on mob type here: some lobby/new-player implementations are
		// treated as living by downstream forks and would silently miss the panel.
